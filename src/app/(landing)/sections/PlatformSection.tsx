...
export default function PlatformSection() {
  return (
    <section className="max-w-[900px] mx-auto w-full px-6 lg:px-10 py-20 space-y-14">
      ...
      <div>
        ...
        <a
          ...
          className="group relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center
            gap-6 sm:gap-8 bg-white border border-[#EBEBEB] rounded-[24px] p-7 sm:p-8 lg:p-10
        ...
        >
          ...
          <div className="relative flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className="text-[10.5px] font-bold tracking-[1.5px] uppercase text-[#B31B38]
                bg-[#FFF0F3] border border-[#FFD5DF] px-2.5 py-1 rounded-full">Live</span>
            </div>
            <p className="mt-2 text-[14px] sm:text-[15px] text-[#666] leading-[1.7] max-w-[480px]">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {TAGS.map(t => (
                <span key={t} className="text-[11.5px] font-medium text-[#888] bg-[#F5F5F5]
      ...
      <div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {STEPS.map(({ n, title, desc }) => (
            <div key={n}
              className="bg-[#FAFAFA] border border-[#EBEBEB] rounded-[20px] p-5
                hover:border-[#F0C8D0] hover:bg-[#FFF8F9] transition-all duration-300">
              <span className="text-[32px] font-bold text-[#F0E0E4] leading-none select-none">{n}</span>
              <h3 className="mt-2 text-[13.5px] font-semibold text-[#0A0A0A]">{title}</h3>
              <p className="mt-1.5 text-[12.5px] text-[#999] leading-[1.6]">{desc}</p>
            </div>
          ))}
        </div>
      </div>
